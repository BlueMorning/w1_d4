...
class TestFriends < MiniTest::Test
  ...
  def test_check_particular_food
    result = check_particular_food?(@person5, "spinach")
  ...
  def test_calculate_total_amount_money
    result = calculate_total_amount_money(@people)
  ...
  def test_loan_lender_to_lendee
    result = loan_lender_to_lendee(@person4, @person2, 20)
    assert_equal([0,22], result)
  end

  # 8. Find the set of everyone's favourite food joined together
  # (hint: concatenate the favourites/things_to_eat arrays together)
  def test_find_people_set_favourite_food
    result = find_people_set_favourite_food(@people)
    assert_equal(["charcuterie", "soup","bread","ratatouille", "stew","spaghetti","spinach"], result)
  end

  # 9. Find people with no friends
  # (hint: return an array, there might be more people in the future with no friends!)
  def test_get_people_with_no_friends
    result = get_people_with_no_friends(@people)
